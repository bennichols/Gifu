...
public class Animator {
    ...
    var loopDuration: NSTimeInterval {
    ...
    private func updateFrameIfNeeded() {
        ...
        store.shouldChangeFrame(with: displayLink.duration) {
            if $0 { self.delegate.animatorHasNewFrame() }
        }
    }
    ...
    func prepareForAnimation(withGIFNamed imageName: String, size: CGSize, contentMode: UIViewContentMode, loopCount: Int = 0, completionHandler: ((Void) -> Void)? = .None) {
        guard let extensionRemoved = imageName.componentsSeparatedByString(".")[safe: 0],
            let imagePath = NSBundle.mainBundle().URLForResource(extensionRemoved, withExtension: "gif"),
            let data = NSData(contentsOfURL: imagePath) else { return }
    ...
    /// - parameter loopCount: Desired number of loops, <= 0 for infinite loop.
    /// - parameter completionHandler: Completion callback function
    func prepareForAnimation(withGIFData imageData: NSData, size: CGSize, contentMode: UIViewContentMode, loopCount: Int = 0, completionHandler: ((Void) -> Void)? = .None) {
    ...
    /// - parameter contentMode: The view content mode to use for the individual frames.
    /// - parameter loopCount: Desired number of loops, <= 0 for infinite loop.
    func animate(withGIFData imageData: NSData, size: CGSize, contentMode: UIViewContentMode, loopCount: Int = 0) {
