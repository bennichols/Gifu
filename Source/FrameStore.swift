...
class FrameStore {
    ...
    var loopDuration: NSTimeInterval = 0
    ...
    init(data: NSData, size: CGSize, contentMode: UIViewContentMode, framePreloadCount: Int, loopCount: Int) {
    ...
    func duration(at index: Int) -> NSTimeInterval {
...
private extension FrameStore {
    ...
    func updatePreloadedFrames() {
        ...
        preloadIndexes(withStartingIndex: currentFrameIndex).forEach { index in
            ...
            animatedFrames[index] = currentAnimatedFrame.frameWithImage(loadFrame(at: index))
        }
    }
    ...
    func incrementTimeSinceLastFrameChange(with duration: NSTimeInterval) {
    ...
    func incrementCurrentFrameIndex() {
        ...
        if isLastLoop(currentLoop) && isLastFrame(currentFrameIndex) {
            isFinished = true
    ...
    func preloadIndexes(withStartingIndex index: Int) -> [Int] {
        ...
        let lastIndex = increment(index, by: bufferFrameCount)
    ...
    func setupAnimatedFrames() {
        ...
        (0..<frameCount).forEach { index in
            ...
            animatedFrames[index] = animatedFrames[index].frameWithImage(loadFrame(at: index))
        }
